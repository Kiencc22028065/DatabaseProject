...
def edit_user(request, cus_id):
    customer = Customer.objects.get(person_id = cus_id)
    user = User.objects.get(email = customer.email)
    if request.method == 'POST':
        if request.POST.get('personName'):
            data = request.POST.get('personName')
            Customer.objects.filter(person_id = cus_id).update(personName = data)
        elif request.POST.get('phoneNumber'):
            data = request.POST.get('phoneNumber')
            Customer.objects.filter(person_id = cus_id).update(phoneNumber = data)
        elif request.POST.get('cusAddress'):
            data = request.POST.get('cusAddress')
            Customer.objects.filter(person_id = cus_id).update(cusAddress = data)
        elif request.POST.get('postCode'):
            data = request.POST.get('postCode')
            Customer.objects.filter(person_id = cus_id).update(postCode = data)
        elif request.POST.get('email'):
            data = request.POST.get('email')
            Customer.objects.filter(person_id = cus_id).update(email = data)
            user.email = data
            user.save()
        else:
